...
public class 과제4_2_가위바위보 {
	...
	public static void main(String[] args) {
		...
		int 플레이어; //플레이어 낸 수 저장하는 변수 역할
		int 컴퓨터;	//컴퓨터 낸 수 저장하는 변수 역할 
		...
		while(true) {
			...
			플레이어 = scanner.nextInt(); // 1. 플레이어에게 입력받은 후
			
			/* 문제풀이 위치 */
				// 컴퓨터는 난수로 자동으로 생성한다.[Math.random() vs new Random();]
				// 1. 랜덤 객체 생성
			
				// 2. 랜덤 객체를 이용한 랜덤 함수 이용[객체가 있으면 메소드/함수/행동 사용 가능]
			
				// 3. 종료 코드를 넣어주는 시점[난수x 승리자판단x]
