	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			if(ch == 1) { // 1. 콜라 선택
				...
				// step2 : 기능 구현
				// 1. 콜라 재고가 있으면 바구니에 담아 없으면 재고없음 출력
				if(콜라재고 > 0) {//재고가 있다
				...
				if(환타재고>0) {환타재고--;		환타바구니++; System.out.println("안내) 환타담았습니다.");}
				else {System.out.println("안내) 환타 재고가 없습니다.[관리자에게 문의]");}
				
			}
			else if(ch == 3) { // 3. 사이다 선택
				if(사이다재고>0) {사이다재고--;		사이다바구니++; System.out.println("안내) 사이다담았습니다.");}
				else {System.out.println("안내) 사이다 재고가 없습니다.[관리자에게 문의]");}
	
			}
			else if(ch == 4) { // 4. 결제 선택
				// 1. 변수/장바구니 내 데이터/개수 출력 "장바구니에 담은 제품만 출력[경우의 수 = 바구니가 0보다 크면]
