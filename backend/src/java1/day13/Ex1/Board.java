	...
	// 1. 필드	: 객체의 데이터 저장하는 곳
		/*
		  - 필드 종류	
		  	...
		  	- 정적필드		: (메소드영역) [static] 프로그램 실행시 우선할당 객체없이 저장되어 있는 필드-사용
		 	
		 - 필드 타입
		 	- 기본 타입	: boolean char ...8가지
		 ...
		 -기타 키워드		
		 	...
		 	- static 	: 프로그램 전체에서 상수 만들기
		 	
		 	- 싱글톤		: 프로그램내 1개만 존재하는 객체
		 		1. private static 사용하는 싱글톤 객체 선언
		 		2. public getInstance( ); 싱글톤 객체 반환해주는 함수
		 		3. private 생성자() 외부로부터 생성자 사용 금지
		 */
	
	// 2. 생성자	: 객체 초기화/[처음값대입] 할 때 사용 -> 무조건 부모 따라감
		/*
		 	- 생성자 사용되는 경우 : 객체 선언할떄
		 		클래스명 객체변수명 =new 생성자명();
		 	...
		 	- 생성자 역할
		 		...
		 		- 2. 유효성검사 할 때 / 계산식이 필요한 값을 필드에 저장할 때
		 		
		 	- 생성자 종류[오버로딩으로 인해 다양하게 선언 가능]
		 	...
		 	- 기타 키워드
		 		...
		 		-this.필드명		: 외부로부터 들어온 매개변수명과 현재 클래스내 필드명과 같을때  식별할 때
		 		-this()			: 현재 클래스의 생성자 호출
	...
	// 3. 메소드	: 객체의 행동/동작 수행할 때 사용
		...
		/*
		 	...
		 	MVC 디자인 패턴 : 기능역할에 따른 구분
		 	 ...
		 	 M: 데이터 모델과 실제 데이터
		 	 	DTO: 데이터 이동 객체 [ 서로 다른 레이어간 이동할때 객체사용 ] //메소드는 수단 
		 	 	...
		 	 	DAO: 데이터 접근 객체 [ 실제 원본 데이터(DB) 접근만]
