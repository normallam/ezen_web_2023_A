...
public class Ex1_반복문 {//class s
	...
	public static void main(String[] args) { //main s
		...
		//[p.127] 실수 	//소수점 뒤에 f 사용이유 : (직접 작성한)리터럴 소수점 =double이기 때문에
		...
		for(int 곱 = 1; 곱<=9 ; 곱++) {System.out.println(2 * 곱 + "  ");}
			// 3. 2~9단 구구단 for 없이
		//System.out.println(2 * 1); System.out.println(2 * 2); ~ System.out.println(2 * 9);
		//System.out.println(9 * 1); ~System.out.println(9 * 9);
			// 1. 2단~9단 for 패턴
		...
		while(run) { //while s
			...
			if(strNum.equals("1")){//만약에 입력이 1이면
				...
				run = false;	//1 : 조건반복변수에 false 대입해서 조건식을 false 만든다. // 위치1
		...
		숫자for : for(int j =1 ; j<=10 ; j++) {
			if(j %2 !=0) {// 홀수 찾기 == 1
				//continue 숫자for; // for 이동// 아래에 있는 코드는 실행x //홀수 출력x
				continue 숫자for;
