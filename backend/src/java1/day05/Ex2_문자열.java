...
public class Ex2_문자열 {
	public static void main(String[] args) {
		...
		String name4 = new String("홍길동");	//21번지
			// " " : 문자열 리터럴이 아닌 new 연산자를 이용한 객체 생성시에는 같을까?
		...
		//[p.157] .charAt( 인덱스 ) 문자열 내에서 특정 문자 추출/꺼내기
		String ssn = "9506241230123";	//문자열 객체 선언 // 글자 13 인덱스 0~12
		...
		//[p.159] .replace() 문자열내 특정 문자열 교체/치환/대체 된 문자열반환
		...
		String ssn2 = "880815-1234567";
				//	
		String secondNum = ssn2.substring(7);	//인덱스7 부터 마지막까지 추출
...
/*
	...
	자바 문자열에서 제공해주는 라이브러리 = 미리 만들어진 함수들
		...
		2. 문자열객체.charAt( 인덱스 )			: 문자열내 해당 인덱스의 문자1개 반환 [첫번째 글자 = 인덱스0]
		3. 문자열객체.length()					: 문자열에서 문자의 개수 반환
			vs 배열명.length
		4. 문자열객체.replace("기존문자열","새로운문자열")	: 문자열내 기존문자열이 존재하면 새로운문자열로 치환해서 반환
		5. 
			문자열객체.substring(시작인덱스)				: 해당 인덱스 부터 마지막인덱스까지 문자열 추출
			문자열객체.substring(시작인덱스,마지막인덱스)	: 시작인덱스부터 마지막 인덱스 전까지 문자열 추출
