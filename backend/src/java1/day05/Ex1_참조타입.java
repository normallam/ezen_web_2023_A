...
public class Ex1_참조타입 {// class s
	...
	//스택구역 = 힙구역
		...
		//[p.151] 객체주소를 참조하고 있지않는 상태 = null *객체(필드,메소드) 사용할 수 없는 상태
		int[] intArray = null;
		// int[] intArray 스택영역에 변수가 메모리 할당 , 참조중인 객체번지x // 힙영역x
		// intArray[0] = 10; //배열내 0번째 인덱스에 10대입??? 오류 // 배열은 힙에서 생성되는데 아직 배열이 생성x
...
/*
	...
	JVM 메모리
		...
		- 함수(프레임)마다 별도구역				- 객체가 생성되는 영역
			...
			- 함수(프레임) 종료되면 제거				- new(힙 메모리를 쓰겠다.) : 타입/클래스 기반으로 새로운 메모리생성후 주소/번지 반환
												
		------------------------------------------------------------------------
		
		int age = 10; //리터럴;(고정된 값)
