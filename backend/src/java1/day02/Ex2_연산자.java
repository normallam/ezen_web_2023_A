...
public class Ex2_연산자 { // class s // 모든 자바 코드는 클래스에서 작성
	...
	public static void main(String[] args) { //main s //모든 실행 코드는 main함수에서
		...
		// 1. 산술연산자 [p.81] : 연산 후 반환 => 기본자료형 데이터
		System.out.println(" x + y : " + (x+y) ); // + 더하기
		System.out.println(" x - y : " + (x-y) ); // - 빼기
		System.out.println(" x * y : " + (x*y) ); // * 곱하기
		System.out.println(" x / 3 : " + (x/3) ); // / 나누기 후 몫(결과값이 int라서 몫처럼 보임)
		System.out.println(" x / 3 : " + (x/3.0) ); // / 나누기 후 몫
		...
		// 3. 논리 연산자 [ p.91 ]
		System.out.println(" x >= 10 and y >= 10 : " + ( x>=10 && y>=10 ) );	//조건이 모두 true 이면 true and false => false
		System.out.println(" x >= 10 or y >= 10 : " + ( x>= 10 || y>=10 ) );	//true or false
