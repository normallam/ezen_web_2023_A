 	...
 	멀티스레드 구현하는 방법
 		...
 		- 상태
 			...
 			2. start() 메소드 이용한 run() 작업코드 실행
 				...
 				실행대기(Runnable) : 실행 대기표 받고 기다리는 중 
 				-> (운영체제가 하드웨어[cpu]를 사용할 수 있는 권한 줄때까지) = cpu 스케줄링
 				
 					반복
 				
 				실행(Running) : 해당 스레드 순서가 되면 cpu 사용할 수 있는 권한[명령어 전달 실행]
 		...
 		- 병렬(여러단위 흐름=다차선도로) 처리 vs 직렬(하나단위 흐름=1차선도로=) 처리
 			...
 			- 컴퓨터의 병렬 처리 방법
 				- 소프트웨어(손에 안잡히는 논리적인 형태)[자바]는 
 				직접적인 하드웨어(cpu,하드웨어같은 물리적인 형태)[cpu,메모리 ]조작할 수 없다.
