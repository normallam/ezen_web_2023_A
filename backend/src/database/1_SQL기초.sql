	...
	데이터베이스 : 데이터[자료] , 베이스[모임]
        ...
        - 용어
				DBA[데이터베이스 관리자]
					- 개발자
				DBMS[데이터베이스 관리 시스템]
					- MYSQL,ORACLE, MARIADB, SQLSERVER - 각 회사병 소프트웨어
        ...
        - SQL : DB 조작/관리/제어 할 때 사용되는 언어/문법
			- 1. DDL [데이터베이스 정의어]
					- 1. create		: 데이터베이스, 테이블(표) 생성
						- create database db명;
                        - create table table명 (필드명 속성명, 필드명 속성명, 필드명 속성명); // 자바랑 순서 반대 (속성명 필드명)
		...
		- 데이터 필드 타입 [mysql기준 *DBMS 조금씩 다름]
			(숫자) : 바이트 단위
            1. 정수
				tinyint[1] 		: +-128~127		-> byte랑 같음 
                smallint[2] 	: +-3만정도 		-> short랑 같음
                mediumint[3] 	: +-8백만정도
                int[4] 			: +-21억정도 
                bigint(8) 		: +-21억이상 900경
			2. 실수
			...
			3. 문자
				( 글자수 최소1~최대255 )  
                char(문자길이)			: 고정길이 배열		char(3) -> 'ab' -> 3바이트	[*저장할 데이터의 글자수가 정확히 정해져 있는경우]
                ( 글자수 최소1~최대65535)
                *varchar(문자길이)		: 가변길이			varchar(3) -> 'ab' -> 2바이트	[*저장할 데이터의 글자수가 정확하지 않은 경우]
                *char 랑 varchar만 소괄호 씀
			4. 대용량 문자
				text (최소1~최대65535)
...
#예1 : 데이터베이스( 여러개의 테이블(표)들이 저장 할 수 있는 공간 - 폴더비슷한 형태 ) 생성
...
#예5 : 만약에 데이터베이스가 존재했을때 삭제[*만약에 데이터베이스 없을 때 삭제명령어 사용하면 오류발생!]
...
create table member(아이디 text, 비밀번호 text);
	...
	# ( )		: 테이블 속성/열/필드 정의 구역
		#( 필드명 타입, 필드명 타입, 필드명 타입, 필드명 타입) : 데이블(표) 제목/속성/열/필드 정의
#예2 : 테이블 확인
select * from member;
#예3 : 테이블 삭제
drop table member;
...
/*
	문제1 : 
		1. 웹개발 하는데 DB저장소 'sqldb1web' 이름으로 DB생성
		2. 해당 DB에 member 테이블 생성
			id(문자열 최대 20)
            password(문자열 최대 10)

*/
# 만약에 생성할 이름의 DB명이 존재하면 삭제
drop database if exists sqldb1web;
# DB 생성
create database sqldb1web;
# DB 사용 선택
use sqldb1web;
# 만약에 생성할 이름의 테이블명이 존재하면 삭제..
drop table if exists member;
# 테이블 생성 (필드명 타입, 필드명 타입)
...
/*
	문제2 :
        ...
        2. 해당 db에 board 테이블 생성
                ...
                게시물작성일	(필드명 : datetime	타입 : 최대 날짜/시간)
*/
