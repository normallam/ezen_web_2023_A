console.log('wishlist열림')
getWishProdutList();	// 만약에 ajax 비동기이면 해당 함수를 실행하고 ajax 결과가 오기전에 아래코드 실행
						// 만약에 ajax 동기이면 해당 함수를 실행하고 ajax 결과가 오기전까지 대기상태
let checkIndex = []; //현재 체크된 인덱스
/*
	1. document.querySelector(선택자); : 해당 선택자의 dom객체로 가져오기				[객체]
...
/*
	0. 제품목록에서의 선택상자 모두 체크기능*/
		...
		checkList[0].addEventListener('click' , (e)=>{
			...
			// checked 속성 이용한 체크여부 확인하기. true : 체크되어 있는 상태 / false : 체크 안된상태
