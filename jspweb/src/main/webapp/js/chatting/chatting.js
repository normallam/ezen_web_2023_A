...
let clientSocket = new WebSocket(`ws://192.168.17.31:80/jspweb/serversokcet/${loginMid}`);
	...
	clientSocket.onopen = e => { 
			...
			//clientSocket.send(msg); // 문제발생 : 해당 메세지를 받는 JAVA는 JSON타입 몰라요 .. 그래서 문자열 타입으로 전송.
...
function onMsg( e ){
	...
	let msgBox = JSON.parse( e.data ); console.log(msgBox); // e.data : 서버로부터 전달받은 내용물 e.data  속성에 있는 상태
