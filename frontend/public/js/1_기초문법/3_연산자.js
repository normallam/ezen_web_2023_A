...
-연산자
		...
		4. 관계연산자: 주로 비교연산자 2개이상일때 사용 , true 혹은 false 출력
			&& 이면서 면서 이고 모두 그리고 and 조건 초등학생이 사탕이면서 초콜릿 먹을래 [ 모두 참true이면 참true]
			||이거나 거나 또는 하나라도 or 조건 초등학생이 사탕이거나 초콜릭 먹을래 [하나라도 참 true이면 참true]
			! 반대 부정 not조건   !true => false / !false=> true

		5. 대입연산자: 
			=대입 [오른쪽 데이터를 왼쪽 대입]
			+= [오른쪽 데이터를 왼쪽에 더한 후에 왼쪽이 대입]
				1. 변수= 10
		...
		7. 삼항연산자: 항 3개 존재하는 연산자, 참 true 혹은 false 출력
				조건 ? 참true : 거짓 false
				-만약에 조건이 true이면 참 실행
...
// 문제1 :
	...
	//1.세 과목 입력받기
let 국어 = Number(prompt('국어 점수 : ')) // 입력 후에 변수에 저장 안하면 사라지니까 변수에 저장/대입
